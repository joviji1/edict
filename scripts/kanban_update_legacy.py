...
def _resolve_execution_org(task, new_state):
    """Resolve the concrete execution org for Next/Doing tasks.

    Prefer an already assigned six-ministry org, then template-suggested targetDept,
    and only fall back to generic state labels when no concrete department is known.
    """
    current_org = str((task or {}).get('org', '') or '').strip()
    target_dept = str((task or {}).get('targetDept', '') or '').strip()
    execution_orgs = {'礼部', '户部', '兵部', '刑部', '工部', '吏部'}
    if current_org in execution_orgs:
        return current_org
    if target_dept in execution_orgs:
        return target_dept
    return STATE_ORG_MAP.get(new_state, current_org)
...
def cmd_state(task_id, new_state, now_text=None):
    ...
    def modifier(tasks):
        ...
        if (old_state[0], new_state) in HIGH_RISK_TRANSITIONS:
            ...
            return tasks
        t['state'] = new_state
        if new_state in ('Next', 'Doing'):
            t['org'] = _resolve_execution_org(t, new_state)
...
def cmd_delegate(task_id, from_agent, to_agent, instruction, return_spec=''):
    ...
    org = _AGENT_LABELS.get(to_agent, to_agent)
