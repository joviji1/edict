...
所以我做了一个**军机处看板**——一个实时监控所有任务流转的 Web 面板。前端现已升级为 React 18 + Vite，后端仍保持 Python 标准库 `http.server`，打开浏览器就能用。
...
还有待批专面板（集中处理 PendingConfirm 审批）、朝堂议政（多官员讨论可视化）、急递铺 / 国史馆、官员总览（功绩排行 + 走势总览）、技能管理、天下要闻、会话监控、上朝仪式（每天首次打开的彩蛋动画）。

**现已升级为 React 前端 + stdlib 后端**：前端使用 React 18 + TypeScript + Vite，后端保持 Python 标准库 `http.server`，部署仍然轻量。
...
后端仍然是 Python 标准库的 `http.server`，不需要 Flask/FastAPI 这类额外框架；前端虽然升级为 React 18 + Vite，但构建产物直接由同一个 server.py 提供静态服务，部署仍然很轻。
...
安装脚本自动创建 12 个 Agent（11 个业务角色 + 1 个兼容角色）所需目录、写入人格文件、注册权限矩阵、重启 Gateway。
...
- **御批模式**：门下省的审议结果已经可以推送飞书 / Telegram，并支持你在看板内或通知链路里准奏 / 封驳；后续可继续补更细的审批编排
- **功过簿**：绩效评分、功绩排行、走势总览与“能臣 / 需训练”标签已经落地，后续继续等真实样本把页面喂满
- **急递铺**：消息流摘要和回查已经有了，接下来补实时连线动画与时间线回放
- **国史馆**：task_memory / autopsy / shared_rule 的统一检索已落地，后续继续补奏折引用溯源链
