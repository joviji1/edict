...
- 2026-05-12 07:53（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：已按用户指令重启 `edict-dashboard.service`，把刚提交的观星台完整聚合字段切入 live，并完成运行态验收。** 变更前已备份 `dashboard/server.py`、`dashboard/dist/`、`edict-dashboard.service` 与 sha256 到 `/root/.hermes/backups/edict-dashboard-restart-observability-live-20260512-075322/`。重启后服务 `active/running`，新 `MainPID=40755`，`ExecStart=/usr/bin/python3 /root/.openclaw/workspace/edict/dashboard/server.py --host 127.0.0.1 --port 7892`，启动时间 `2026-05-12 07:53:22 CST`；`/healthz` 返回 `status=ok`。带 auth live smoke 访问 `/api/observability-panel?limit=3&q=token` 返回 HTTP 200 / `ok=true`，且新增字段已进入 live 返回体：`search.groups`、`logs.streams`、`cron.summary`、`tokens.summary.byAgent/byModel/alerts` 均存在；`sources.consistent=true`，三面计数均为 20，状态 `Cancelled=15 / Done=5`；gateway token 只返回脱敏值，未泄露明文。journal 近两分钟仅见正常 stop/start、JWT 认证启用、定时巡检启动，未见 traceback。边界：本轮只重启 dashboard 服务，没有重启 backend/loop，没有写 DB、没有改任务状态、没有对外发送。
- 2026-05-12 07:01（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：已阅读公众号《三省六部制2.0：用Profile做真正的进程隔离》，并排查本地 edict 是否已使用 Hermes Profile。结论：本地 Hermes Profile 机制没有被 edict 使用；当前 edict 使用的是 OpenClaw 自己的 agent 目录、workspace、SOUL.md、sessionKey 隔离，属于“OpenClaw 多 agent/session/workspace 隔离”，不是文章里的 `~/.hermes/profiles/<department>` 进程级 Profile 隔离。** 文章核心主张是把原先 prompt 内分工升级为 Hermes Profile：每个部门独立 `HERMES_HOME/.env/SOUL.md/memories/sessions`，共享 skills，并用 `hermes profile create zhongshu --clone`、`hermes chat -p zhongshu` 这类命令运行。现场证据：1）`hermes profile list` 只显示 `default gpt-5.5 running`，没有 `zhongshu/menxia/shangshu/gongbu/hubu/libu/bingbu` 等 Profile；2）`/root/.hermes/profiles` 当前不存在；3）代码/文档精确搜索未命中 `hermes profile`、`profile create`、`~/.hermes/profiles`、`HERMES_PROFILE`，`--profile` 命中只来自依赖包/Vite 噪音；4）edict 现有部门隔离证据集中在 OpenClaw：`/root/.openclaw/agents/{taizi,zhongshu,menxia,shangshu,...}/sessions/sessions.json`、`/root/.openclaw/workspace-{taizi,zhongshu,menxia,...}/SOUL.md/AGENTS.md`，以及 `data/agent_config.json` 里每个 agent 的 `workspace=/root/.openclaw/workspace-xxx` 与模型配置；5）dashboard 唤醒链 `dashboard/server.py::wake_agent()` 使用 `openclaw gateway call agent --params {agentId, sessionKey: agent:<id>:edict-dispatch}`，测试也锁定 `agent:taizi:edict-dispatch`，这说明当前隔离主轴是 OpenClaw gateway sessionKey，而非 Hermes `-p/--profile`。风险判断：文章思路对 edict 有参考价值，但不能直接按“已经用了 Profile”汇报；若后续要升级，应作为新架构支线做 Hermes Profile ↔ edict 部门映射设计，并先处理 gateway、Feishu、session 归档、技能共享、密钥隔离和迁移/回滚边界，不能在当前生产链上直接批量 `profile create` 后切流。
- 2026-05-12 06:39（北京时间）：**外部基础设施 / metapi on hep：已按“三方日志”为主证据做第二轮收紧，核心 gpt-5.4/gpt-5.5 公开入口实测恢复稳定，且 watchdog 的复探盲区已修。** 本轮证据链明确改为三方交叉：1）metapi `proxy_logs` 最近窗口显示核心 `gpt-5.4` 并非整体不可用，最新连续成功落到 `chshapi.cn account37 channel31910` 与 `freemodel account69 channel31836`；2）Hermes `agent/errors/gateway.log` 仍可追到 2026-05-11 多次 `custom:metapi gpt-5.4` 的 `HTTP 503: No available channels for this model`、403 blocked、429、504/connection error；3）飞书聊天记录里用户多次“切换 gpt-5.4 / 再次检查 metapi / 根据日志排除问题”，与 proxy 失败窗口吻合。结论不是“模型池整体死”，而是坏通道/错误探活继续污染判断：`metapi-site-local-watchdog.py` 到期复探原先只拿 `sites.url`，导致 freemodel 这类真实 endpoint 为 `https://api.freemodel.dev` 的站点被误打到 `https://freemodel.dev/models`，此前日志里反复出现 freemodel 403、windhub 非 JSON；现已备份脚本到 `/home/joviji/metapi/recovery_backups/metapi-site-local-watchdog.py.bak-endpoint-probe-20260512_063554`，并改为优先读取 enabled `site_api_endpoints.url`，再 fallback 到 `sites.url`，dry-run 通过且当前 `actions_count=0`。同时已在 live DB 变更前由容器内脚本备份 `/app/data/hub.db.metapi-three-log-tighten-2026-05-11T22-36-25-183Z`，随后定向清理：清掉 freemodel endpoint `id=97` 因 `/v1/responses` 504 留下的 stale `cooldown_until/last_failed_at/last_failure_reason`；把近期失败主导的目标通道列表继续摘流，其中实际变更 5 条，保持 `enabled=0 / manual_override=1 / priority=-100 / weight=1 / cooldown_until=NULL`。验证证据：`python3 -m py_compile` 同时通过 `metapi-auto-recover.py` 与 `metapi-site-local-watchdog.py`；auto-recover dry-run 仍有 46 个候选/16 个 selected，但主要被 `skip_route_only_unclassified_billing_policy`、`skip_no_api_token`、`probe_failed` 等保守策略拦住，没有大范围复活旧坏通道；public metapi smoke 真实请求 `gpt-5.4` HTTP 200（约 2.6s，返回 pong，最新 proxy 落 `freemodel account69 channel31836`），`gpt-5.5` HTTP 200（约 1.8s，最新 proxy 落 `chshapi.cn account37 channel31912`）。最新 GPT enabled 路由面只保留核心可承流候选：`gpt-5.4` 为 chshapi/freemodel 两条，`gpt-5.5` 为 chshapi/freemodel/moyuu；已无 target enabled route 指向 disabled/expired 账号。边界：本轮没有重启 metapi，没有改 Hermes 配置，没有恢复 shark/小米/windhub 旧坏通道；后续继续按“双层守护”：5 分钟 watchdog 只读 proxy_logs + 到期定向 endpoint 探活，30 分钟 auto-recover 保守处理 route health。
- 2026-05-12 06:08（北京时间）：**外部基础设施 / metapi on hep：已按 metapi proxy_logs + Hermes agent/gateway 日志 + 飞书切模型报错，把 auto-recover 再收紧一刀，并真实 apply 关闭会漏流的脏通道。** 证据面先看最近 48h proxy_logs：`gpt-5.4` 主链有 4291 次成功，但失败集中在旧坏通道与上游兼容路径；典型失败为 moyuu/Gundam `gpt-5.4` 403、abrdns `gpt-5.4` 503/429/站点地址不可用、小米 `mimo-v2.5-pro` 429/404/地址不可用、windhub MiniMax/Mimo 403，以及 freemodel account70 401/502/504。Hermes 自身日志同时显示 2026-05-11 多次 `custom:metapi gpt-5.4` 在 cron/会话/标题/压缩路径上报 `HTTP 503: No available channels for this model`、403 blocked、429 限流和 504/connection error；飞书聊天记录中用户多次触发“切换 gpt-5.4 / 再次检查 metapi”，与 proxy 失败窗口吻合。根因不是“模型都不可用”，而是路由池里仍残留 `enabled=1` 但实际不该承流的通道：disabled site/account 的 Faker 两条 GPT channel、expired 的 freemodel account70 两条 GPT channel，以及 priority=-100/weight=1 仍 enabled 的小米 `mimo-v2.5-pro`。变更前已备份脚本、policy、watchdog 与 live DB 到 `/home/joviji/metapi/recovery_backups/optimize-logs-hermes-feishu-20260512-060502/`，apply 前脚本自身又备份 DB 到 `/home/joviji/metapi/recovery_backups/metapi-auto-recover-v2-20260512_060651.hub.db`。脚本修改点：`metapi-auto-recover.py` 的 route-health 从只盯 `gpt-5.4` 扩到 `gpt-5.4/gpt-5.5/mimo-v2.5-pro/MiniMax-M2.7`；坏通道隔离阈值从失败 80 次降到 20 次但仍要求 `total>=20 且 fail_ratio>=0.85`；新增两类优先动作 `disable_ineligible_channel` 与 `disable_quarantined_enabled_channel`，确保 `site/account disabled/expired` 或 `enabled=1 + priority<=-100/weight<=1` 的目标通道不会在 weighted/fallback 压力下继续漏流。已通过 `python3 -m py_compile`；dry-run 先显示 6 个 route_health 候选，其中包含小米 31776、freemodel70 31839/31838、Faker 31795/31920；随后真实 apply 关闭这 5 条通道，复核均为 `enabled=0 / manual_override=1 / priority=-100 / weight=1 / cooldown_until=null`。二次 dry-run 后 route_health 候选只剩 1 个，且目标模型下已无 enabled ineligible channel。public metapi smoke 复验：`gpt-5.4` HTTP 200（落 chshapi account37 channel31910）、`gpt-5.5` HTTP 200（落 chshapi account37 channel31912）、`minimaxai/minimax-m2.7` HTTP 200（最终落 nvidia account19 channel32451；中间 shark 一条 401 被 fallback 后成功）。边界：本轮没有重启 metapi，没有强行恢复 shark/小米/windhub 等历史坏通道，也没有改 Hermes 配置；只是把脚本的“坏/不合格通道摘流”能力扩到用户实际切换模型会踩到的范围，避免路由池看着有通道但运行... [truncated]
- 2026-05-12 05:48（北京时间）：**外部基础设施 / metapi on hep：再次巡检结果是核心承流正常，当前不需要重启或大范围写库；GPT 主链已由 chshapi/freemodel 稳定承流，MiniMax/Mimo 仍按前序保守策略处理。** 本轮先核 hep 运行态：`metapi-metapi-1` 已运行 8 小时，`new-api/new-api-postgres/new-api-redis` healthy/运行中；cron 仍挂两层守护：`*/30` 的 `/home/joviji/metapi/recovery_backups/metapi-auto-recover.py` 与 `*/5` 的 `/home/joviji/metapi/recovery_backups/metapi-site-local-watchdog.py --apply`。最近守护日志显示 auto-recover 在 05:30（北京时间）真实 apply 并写出备份 `/home/joviji/metapi/recovery_backups/metapi-auto-recover-v2-20260512_053001.hub.db`，watchdog 05:35/05:40/05:45 三跳均 `actions_count=0`，说明没有新的站点级摘流动作。proxy_logs 最近 2 小时核心证据：`chshapi.cn / gpt-5.4` 成功 98 次，`freemodel / gpt-5.4` 成功 46 次；只有 `chshapi gpt-5.4` 早些时候 2 条 failed/http_status=0、`openrouter gpt-5.4` 2 条 403，近 30 分钟失败窗口为空。目标路由面：`gpt-5.4` 有 4 条 enabled、2 条 eligible，当前实际选中 `chshapi account37 channel31910` 与 `freemodel account69 channel31836`；`gpt-5.5` 有 6 条 enabled、4 条 eligible，核心 `chshapi/freemodel/moyuu` 仍在；`minimaxai/minimax-m2.7` 有 3 条 eligible，最近实测走 `nvidia account19 channel32451`；`mimo-v2.5-pro` 仍只有小米旧通道且 cooldown/失败主导，不应自动恢复；`MiniMax-M2.7` 短名 route 当前无 eligible 站点，继续按“暂不可用/需新上游”处理。对外 public metapi smoke 使用现有 `custom:metapi` key 实测：`gpt-5.4` HTTP 200（约 2.9s）、`gpt-5.5` HTTP 200（约 1.7s）、`minimaxai/minimax-m2.7` HTTP 200（约 6.9s）；随后 proxy 最新日志确认三条分别落到 `chshapi` 与 `nvidia` 并成功，其中 minimax 先有一条 `/v1/responses` 404 后自动转 `/v1/chat/completions` 成功，不影响最终 chat smoke。auto-recover 当前 dry-run 仍有 41 个候选/16 个 selected，但大多被保守策略拦住：windhub/moyuu/huainova/Gundam/abrdns/openrouter 等 `unknown billing` 不做 route-only 复活；`shark account65` models 可见但 chat `gpt-5.4/gpt-5.5` 均 429 `model_cooldown`，继续 `skip_subscription_models_ok_chat_failed`；`shark account66` 缺 api_token，继续跳过；小米 `mimo-v2.5-pro` models 可见但近期失败主导，继续跳过。边界：本轮只做只读巡检 + public smoke，没有改 DB、没有重启、没有对外发送。建议维持当前双层守护，暂不把被 quarantine 的 shark/小米/windhub 等通道放回；真正要补的是 MiniMax 短名 route 的新可靠上游，而不是强行复活旧坏通道。
- 2026-05-11 19:36（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：已补完 dashboard 重启后的系统级验收，并顺手收掉一条会把监控误报成未收口的历史 session 索引噪音。** 本轮不是只看单接口 smoke，而是按运行态整体验收复核：`curl http://127.0.0.1:7892/healthz`、`http://127.0.0.1:18000/health`、`http://127.0.0.1:18000/api/admin/health/deep` 全部返回 `status=ok`；`bash scripts/doctor_edict.sh` 返回 `PASS=32 WARN=0 FAIL=0`。三面一致继续成立：backend API、`data/tasks_source.json`、`data/live_status.json` 均为 20 条，状态分布均是 `Done=5 / Cancelled=15`，且 `tasks_source/live.tasks` 的 `sourceLayer` 全为 `backend_export`，`live_status.taskSource=backend_api_export`，`tasks_backend_export_meta.exportedAt=2026-05-11T11:34:01.018040+00:00`，说明 dashboard 重启没有打穿 backend export 主链。session monitor 初验仍报 1 条 medium：`agent:zhongshu:edict-dispatch` 被判定 `long_running 84 分钟`；继续下挖后确认这不是新故障，而是历史索引噪音——`/root/.openclaw/agents/zhongshu/sessions/sessions.json` 里该会话 `metadata` 早已有 `closedBy/noiseClearedBy=hermes-operational-verification`，但 `status` 还残留为 `running`，所以监控还在误报。已先备份 `/root/.openclaw/agents/zhongshu/sessions/sessions.json` 与对应 transcript 到 `/root/.hermes/backups/zhongshu-session-running-fix-20260511-193554/`，随后只修索引状态为 `done`，保留 transcript 证据不篡改内容，并更新 `closedReason/noiseClearedReason` 为“历史索引误留 running，经系统验收修正”。复验 `python3 scripts/monitor_openclaw_sessions.py --agents taizi zhongshu menxia shangshu --json` 已恢复为 `high=0 / medium=0 / low=0 / issues=[]`。本轮新增结构化证据已写入 `docs/state/verification-ledger.json` 顶部 entry `VERIF-20260511-1936-DASHBOARD-LIVE-PRECHECK-RESTART`；期间我一度用大块 patch 把 ledger JSON 打坏，随后已用 `git show HEAD:docs/state/verification-ledger.json` 取干净基线重建，再插入新 entry，并用 `python3 -m json.tool docs/state/verification-ledger.json` 重新验证通过。边界：本轮除了修正 `zhongshu` 历史 session 索引外，没有改 backend 任务、没有改 DB、没有发外部消息。
- 2026-05-11 19:24（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：已重启 `edict-dashboard.service`，把 18:09 后新增的更外层 auth precheck 兜底也切入 live，并完成运行态验收。** 变更前已备份 `dashboard/server.py`、`dashboard/auth.py`、`tests/test_server.py`、`docs/current-progress-board.md` 与 `/etc/systemd/system/edict-dashboard.service` 到 `/root/.hermes/backups/edict-dashboard-precheck-live-20260511-192233/`，备份内含 `service.before.txt` 与 `sha256.before.txt`。随后执行 `systemctl restart edict-dashboard.service`，服务当前 `active/running`，新 `MainPID=1239890`，`ExecStart=/usr/bin/python3 /root/.openclaw/workspace/edict/dashboard/server.py --host 127.0.0.1 --port 7892`，进程启动时间 `2026-05-11 19:22:55 CST`，已晚于源码 mtime `2026-05-11 18:09:43 CST`，因此这次不再是“源码晚于进程”的半加载状态。运行态证据：`/healthz` HTTP 200；`/api/auth/status` 返回 `enabled=true/configured=true`；坏 Bearer 访问 `/api/live-status` 返回 HTTP 401 JSON `{"ok": false, "error": "未登录或会话已过期"}`，不是断连接；登录后访问 `/api/observability-panel?limit=3&q=token` 返回 HTTP 200 / `ok=true / hasSources=true / tokenRedacted=true`，sources 三面一致：`tasksSource.count=20 byState={Done:5, Cancelled:15}`、`liveStatus.count=20 taskSource=backend_api_export`、`backendDb.count=20 byState={Done:5, Cancelled:15} ok=true`、`consistent=true`。字节码证据：`dashboard/__pycache__/server.cpython-38.pyc` 已包含 `_collect_observability_sources`、`sources`、`Dashboard auth token verification failed`、`Dashboard auth precheck failed`、`observability-panel failed`，说明 verify_token 兜底、requires_auth/extract_token 外层 precheck 兜底与 observability 路由兜底都已进入 Python 3.8 live 字节码。补跑验证：`/usr/bin/python3 -m py_compile dashboard/server.py tests/test_server.py` 通过；`python3 -m pytest tests/test_server.py::test_check_auth_verifier_exception_returns_401 tests/test_server.py::test_check_auth_requires_auth_exception_returns_401 tests/test_server.py::test_get_observability_panel_summarizes_search_logs_cron_session_token -q` → `3 passed in 0.28s`。journal 自 19:22:50 起显示服务正常 stop/start、JWT 认证启用、定时巡检启动、启动恢复无需恢复、`refresh_live_data.py` 更新 `live_status.json (20 tasks)`，未见 traceback。边界：本轮只重启 d... [truncated]
- 2026-05-11 19:17（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：已确认 live `edict-dashboard.service` 进程加载的是“认证异常兜底 + 观星台 sources”这版代码；但当前 live 进程尚未加载 18:09 后新增的更外层 `requires_auth` precheck 兜底。** 现场核查口径：systemd 显示 `edict-dashboard.service` 当前 `active/running`，`MainPID=3071319`，`ExecStart=/usr/bin/python3 /root/.openclaw/workspace/edict/dashboard/server.py --host 127.0.0.1 --port 7892`，`WorkingDirectory=/root/.openclaw/workspace/edict`，进程启动时间为 2026-05-11 15:17:03 CST，服务 unit 位于 `/etc/systemd/system/edict-dashboard.service`。源码/备份交叉验证：15:16 重启前备份 `/root/.hermes/backups/edict-dashboard-sources-live-20260511-151603/server.py` 已包含 `verify_token(token)` 外层 `try/except Exception`、日志 `Dashboard auth token verification failed`、`/api/observability-panel` 路由、`observability-panel failed` 兜底和 `sources: _collect_observability_sources(tasks)`；这与 15:17 live 重启载入的代码面一致。运行态验证：`/healthz` HTTP 200；`/api/auth/status` 返回 `enabled=true/configured=true`；故意用坏 Bearer 访问 `/api/live-status` 返回 HTTP 401 JSON `{"ok": false, "error": "未登录或会话已过期"}`，不是断连接；登录后访问 `/api/observability-panel?limit=3&q=token` 返回 HTTP 200 / `ok=true / hasSources=true`，sources 三面一致：`tasksSource.count=20 byState={Done:5, Cancelled:15}`、`liveStatus.count=20 taskSource=backend_api_export`、`backendDb.count=20 byState={Done:5, Cancelled:15} ok=true`、`consistent=true`。验证命令也已补跑：`/usr/bin/python3 -m py_compile dashboard/server.py` 通过；`python3 -m pytest tests/test_server.py::test_check_auth_verifier_exception_returns_401 tests/test_server.py::test_check_auth_requires_auth_exception_returns_401 tests/test_server.py::test_get_observability_panel_summarizes_search_logs_cron_session_token -q` → `3 passed in 0.25s`。边界说明：当前工作区 `dashboard/server.py` mtime 为 18:09:43、sha256 `2f10e7867a45952e0d7a1778a8881842f935a032206724060912eed67800abc2`，比 live 进程启动晚；其中新增的 `Dashboard auth precheck failed` 外层 precheck 兜底只在当前源码和 Python 3.8 pyc 中可见，不应宣称 15:17 进程已经加载。准确结论是：**live 已加载用户点名的 verify_token 认证异常兜底与 observability/sources 兜底版本；若要把 18:09 后更外层 precheck 兜底也切入 live，需... [truncated]
- 2026-05-11 18:39（北京时间）：**外部基础设施 / metapi on hep：已把 P0 包月站恢复口径从“models 可见即补 route”升级为“本地计费策略标注 + models/chat 双成功才放回”，并定向恢复 freemodel 一个真实可用通道。** 现场先复核 live DB：metapi 原生 schema 只有 `sites/accounts/route_channels/token_routes/model_availability` 等字段，没有 `billing_type/subscription_type/plan/expire_at` 这类原生包月/包周/按量字段，因此本轮新增 overlay policy `/home/joviji/metapi/recovery_backups/metapi-route-policy.json`，默认把 `chshapi.cn / freemodel / shark` 标为 `monthly + p0`，其他站点按普通/unknown 处理。三家状态边界已确认：`chshapi.cn account37` active，`gpt-5.4/gpt-5.5` route enabled 且权重 100，近 6h `gpt-5.4` 为 279 成功 / 61 失败，仍可承流；`freemodel 69/70` site/account active 且 `/v1/models` healthy，但 GPT channel 原先均为 `enabled=0 / manual_override=1 / priority=-100 / weight=1`；`shark 64/65/66` models healthy，但 GPT channel 也在 quarantine，且 `shark account65 gpt-5.4` 近 6h 36 次全 429。`gpt-5.4-mini` 的 `token_routes.enabled=0`，本轮继续硬拦，没有强开。脚本侧已备份并修改 `/home/joviji/metapi/recovery_backups/metapi-auto-recover.py`：新增 `load_policy()/billing_type()/is_subscription_site()`，候选层跳过 `sites.status='disabled'`，普通站点若近期 proxy 失败主导则 `skip_ordinary_recent_failure_dominant`，P0/包月站若遇到 quarantine/manual/cooldown blocker 不再 route-only 复活，而是必须先 `/v1/models` 成功再对目标模型跑最小 `/v1/chat/completions` smoke；只有 chat 也成功才允许恢复为 `priority=0 / weight=100 / manual_override=1`。dry-run 证据：`shark account65` models 成功但 chat `gpt-5.4/gpt-5.5` 均 HTTP 429 `model_cooldown`，结果 `skip_subscription_models_ok_chat_failed`；`freemodel account69` models 成功但 chat 超时，跳过；`freemodel account70` models 成功且 `gpt-5.4` chat 返回 200/pong，结果 `would_repair_subscription_live_probe`。随后没有跑全量 apply，避免顺手改到 openrouter 等旁支，而是通过容器内 `better-sqlite3` 定向恢复 `freemodel account70`：`gpt-5.4 channel 31839` 与 `gpt-5.5 channel 31838` 均从 quarantine 改为 `enabled=1 / manual_override=1 / priority=0 / weight=100 / cooldown_until=null / last_fail_at=null`，并刷新 `model_availability.checked_at=2026-05-11 10:37:36`；DB 备份为 `/home/joviji/metapi/recovery_backups/targeted-p0-freemodel70-20260511_183736.hub.db`。验证：public metapi `PO... [truncated]
- 2026-05-11 17:22（北京时间）：**外部基础设施 / metapi on hep：已确认 17:00 / 17:05 两个真实 cron 下一跳都已触发，并把 `metapi-auto-recover.py` 的恢复阈值再收紧一档，专门压住 shark 一类历史 snapshot 残留继续晃。** 实时核对 `cron.service` journal：17:00 同时触发 `(joviji) CMD (/home/joviji/metapi/recovery_backups/metapi-auto-recover.py ...)` 与 `(joviji) CMD (/home/joviji/metapi/recovery_backups/metapi-site-local-watchdog.py --apply ...)`，17:05 再次触发 watchdog；说明不是“脚本手动跑通”，而是 cron 下一跳真实执行。对应日志也已刷新：`metapi-auto-recover.log` 最新 `checked_at=2026-05-11T09:00:09Z`，`backup_db=/home/joviji/metapi/recovery_backups/metapi-auto-recover-v2-20260511_170001.hub.db`；`metapi-site-local-watchdog.log` 最新 `checked_at=2026-05-11T09:05:01Z` 且本轮 `actions_count=0`，说明 17:05 那跳 watchdog 已正常空跑收口。随后继续收紧 `metapi-auto-recover.py` 两处口径：1）`match_target_model()/route_for_model()` 从“短串包含”改成**严格匹配**：只允许 exact match 或 provider-qualified tail match，禁止 `gpt-5.4` 吞掉 `gpt-5.4-nano`、禁止把 `gpt-5.3-codex` 误算进目标集合；2）普通 probe-success repair 不再对 probe 命中的所有 target model 一把梭清 stale+补 route，而是只处理“该账号当前确实缺 enabled route 的目标模型”，若账号 active 且 runtime 非 unhealthy、probe 成功但 `missing_target_models=[]`，则直接 `skip_probe_ok_no_missing_target_routes`。验证上，新的 dry-run / apply 已各跑一轮，最新 apply 备份为 `/home/joviji/metapi/recovery_backups/metapi-auto-recover-v2-20260511_172110.hub.db`；`shark 64/65/66`、`windhub 12/48/54`、`moyuu 39/40`、`Gundam 68` 当前都不再自动补回历史 snapshot，而是统一落成 `skip_route_only_requires_live_probe`，blockers 明确指向 `manual_override=1 / priority=-100 / 近 24h proxy 失败主导`。这轮最关键的收口是：`gpt-5.4-nano` 已不再被错误归入 `gpt-5.4` 目标族，`shark#65/#66` 也不会再因为 `/v1/models` 可见 `gpt-5.4/gpt-5.5/minimax` 就自动把历史 quarantine 通道补回。边界：当前 `auto-recover` 已从“历史模型可见就补”收紧为“严格模型匹配 + 仅补当前缺 route + quarantine/失败证据优先阻断”，但仍然没有对每个恢复候选做 chat smoke；因此它现在适合做保守守护，不适合代替完整业务模型可用性验收。后续若还要再压，可把 `route_only_requires_live_probe` 升级成“必须 `/v1/models` + chat smoke 双成功才允许恢复特定模型路由”。
- 2026-05-11 16:52（北京时间）：**外部基础设施 / metapi on hep：已升级到官方最新镜像，并把本地 overlay 收成更保守的“日志驱动 + 定向恢复 + 防误删路由绑定”闭环。** 变更前已备份 compose/env/live DB/sidecar 到 `/home/joviji/metapi/recovery_backups/metapi-official-upgrade-pre-20260511_163235/`；随后执行 `docker compose pull metapi && docker compose up -d metapi`，当前容器为 `1467078763/metapi:latest sha256:1a42e6d21c28623e8d6885c2af400b9f15f7dd2e936d2f242ae2993eaf5df6b3`，`metapi-metapi-1` running，映射 `0.0.0.0:4005->4000`。升级后管理面验证：`/api/sites` HTTP 200 且 53 个站点，`/api/accounts` HTTP 200 且 65 个账号，`/api/events?limit=1` HTTP 200；容器日志显示 DB migration complete，且有真实 `/v1/chat/completions` 200。overlay 侧先修 `metapi-auto-recover.py` 的 route-only 打架问题：新增 `route_only_blockers()`，对 `manual_override=1 / priority<=-100 / cooldown / 近 24h proxy 失败主导` 的通道，禁止仅凭历史 `model_availability` 自动复活，必须走 live probe；验证 dry-run 中 windhub / huainova / moyuu / Gundam 由原先 `would_repair_route_from_model_availability` 变为 `skip_route_only_requires_live_probe`。随后执行 guarded apply，备份 DB 到 `/home/joviji/metapi/recovery_backups/metapi-auto-recover-v2-20260511_164615.hub.db`，实际隔离 `windhub channel 32131` 与 `abrdns channel 31923`，并保持既有坏通道 `moyuu 29103/30936`、`Gundam 29356` 为 `enabled=0 / manual_override=1 / priority=-100 / weight=1`；只补了无失败拦截证据的安全 route：`shark#66 minimaxai/minimax-m2.7 -> channel 34417`、`SunRouter#67 gpt-5.4-nano -> channel 34418`。watchdog 侧已把 `/home/joviji/metapi/recovery_backups/metapi-site-local-watchdog.py` 改为更保守恢复：5 分钟 local-only 读 `proxy_logs` 最近 10 分钟；异常站点进入 sleep 后 snapshot 摘流；到期才定向 `/v1/models` 探活；恢复时同时要求“模型列表包含该 channel 的模型”且“该 channel 近 24h 不是失败主导”，避免 `/v1/models` 可见但 chat 长期 403 的通道被误恢复。一次 apply 备份到 `/home/joviji/metapi/recovery_backups/metapi-site-watchdog-20260511_164939.hub.db`，moyuu 恢复了非 gpt-5.4 的 8 条 snapshot，但继续跳过 gpt-5.4 坏通道 `29103/30936`；shark 只恢复模型列表确认的 minimax/glm/deepseek/qwen 相关通道，跳过未确认的 gpt/claude/kimi 历史 snapshot；Gundam 探活仍 HTTP 403，下一次探活排到 09:49Z。cron 现状已落地：`*/30 * * * * /home/joviji/metapi/recovery_backups/metapi-auto-recover.py ...` 与 `*/5 * * *... [truncated]
- 2026-05-11 15:33（北京时间）：**外部基础设施 / metapi on hep：复核 shark / freemodel 路由“不显示/衔接不顺”后确认，问题不是单纯“站点没了”，而是模型可见、路由启用、账号健康、真实承流四层互相打架。** 只读脚本 `/tmp/metapi_route_audit.py` 读取 live DB `/home/joviji/metapi/data/hub.db`：当前 metapi 有 `sites=53 / accounts=65 / token_routes=144 / route_channels=204 / model_availability=1122 / proxy_logs=81352`；`metapi-auto-recover.py` 存在且可执行，cron 为 `*/30 * * * * /home/joviji/metapi/recovery_backups/metapi-auto-recover.py ...`，日志最新到 15:30。目标账号状态：`shark` 三个账号 `64/65/66` 均 site/account active 且 runtimeHealth healthy；`freemodel` 两个账号 `69/70` 虽有历史 `model_availability` 命中 `gpt-5.4/gpt-5.4-mini/gpt-5.5`，但当前 account 均为 `expired`，runtimeHealth 为 `访问令牌失效：HTTP 401`，所以属于“历史模型可见但当前不可承流”。目标路由面：`gpt-5.4` 不是没 shark/freemodel，实际 `chshapi account37` P0 enabled，`shark account65/66` P1 enabled（其中 65 有 cooldown，66 可用），`freemodel 69/70` P2 enabled 但账号 expired/cooldown；`gpt-5.5` 下 shark 64/65/66 与 freemodel 69/70 都有 enabled channel，但 freemodel 仍因账号 expired 被运行时过滤；`gpt-5.4-mini` route 本身 `enabled=0`，所以即使 freemodel 有 channel 也不会承流。最近 24h proxy 证据显示：`shark account65 + gpt-5.4` 有 379 次成功，但也有 132 次 429 冷却；`freemodel 69/70 + gpt-5.4` 仍有少量成功（28/25 次），但随后出现 401/502/504/terminated 并被标 expired；`chshapi account37` 当前仍是主承流，gpt-5.4 有 673 次成功但也有 responses 400（请求体缺 input content）、401 token invalidated、429/503/504 等噪音。关键新发现：`metapi-auto-recover.py` 的 route-only 分支会在 `target_models_available_but_no_enabled_route + account.status=active` 时直接 `inserted_channel/reenabled_existing_channel`，没有充分尊重前序 `quarantined`/失败证据；日志里同一批站点反复出现 `quarantined` 与 `repaired_route_from_model_availability` 交替，例如 windhub/moyuu/Gundam/abrdns/shark 等。这解释了“站点很多但衔接不顺”：脚本确实起作用，但过度补路由，把部分之前摘掉的坏通道又补回，造成 UI/路由池看起来丰富，实际运行时仍被 expired/cooldown/proxy 失败过滤或拖慢。边界：本轮没有写 DB、没有改 route、没有重启 metapi；已把该 auto-recover 打架坑补进 `metapi-health-guard` skill。下一步若修，应先备份 DB 和脚本，再给 auto-recover route-only 分支加保护：不自动复活 `manual_override/priority<=-100/近期失败占主导/cooldown` 通道，必要时先 live probe 成功再补路由。

- 2026-05-12 07:44（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：继续收口观星台与 safe_update 提交包，已补齐观星台返回体完整分组字段、Token 聚合维度与 safe_update 备份稳定性，并完成代码/构建/live smoke 验证。** 本轮先定位两个真实缺口：1）`scripts/safe_update.sh prepare` 调 `memory_backup.sh` 时会连带尝试备份 workspace 整体，测试场景输出目录位于临时 safe-update 目录时容易放大备份面并出现假红；现已给 `memory_backup.sh` 增加 `EDICT_MEMORY_BACKUP_SKIP_WORKSPACE=1` 开关，safe_update prepare 默认跳过 workspace 整体备份，只保留关键 memory/data/config/systemd/git 指针备份。2）观星台后端 `get_observability_panel()` 测试契约要求 `search.groups`、`logs.streams`、`cron.summary/nextRunHint`、`sessions.status/messageCount/markers`、`tokens.summary.byAgent/byModel/byTask/alerts` 与 `governanceSamples` 统计，代码此前只返回基础 items；现已补齐这些只读聚合字段，且 gateway token 仍只返回脱敏值。同步 `.gitignore` 增加 `.cron_script_status.*`，避免本地 cron script 状态文件进入提交面。验证证据：`bash -n scripts/memory_backup.sh scripts/safe_update.sh` 通过；`python3 -m py_compile dashboard/server.py scripts/smoke_observability_auth.py edict/backend/app/api/tasks.py edict/backend/app/services/task_service.py tests/test_server.py tests/test_smoke_observability_auth.py tests/test_backend_progress_resource_fields.py` 通过；聚焦回归 `python3 -m pytest -q tests/test_safe_update_script.py tests/test_smoke_observability_auth.py tests/test_backend_progress_resource_fields.py tests/test_server.py::test_check_auth_verifier_exception_returns_401 tests/test_server.py::test_check_auth_requires_auth_exception_returns_401 tests/test_server.py::test_get_observability_panel_summarizes_search_logs_cron_session_token tests/test_server.py::test_observability_panel_route_returns_payload` → `12 passed in 3.21s`；`cd edict/frontend && npm run build` 通过，产物为 `dashboard/dist/assets/index-Be6XNtVi.js` 与 `index-CozhF3U3.css`；带 auth live smoke `EDICT_DASHBOARD_PASSWORD=*** python3 scripts/smoke_observability_auth.py --base-url http://127.0.0.1:7892 --query token --limit 3` 返回 HTTP 200 / `ok=true` / `tokenRedacted=true` / `sources.consistent=true`，三面均为 20 条，状态 `Cancelled=15 / Done=5`。边界：本轮没有重启 `edict-dashboard.service`，因此新增的更细分聚合字段目前是代码/测试/构建已收口，live smoke 只证明现有服务 auth 与 sources 链路仍健康；切 live 需后续按变更窗口重启 dashboard。
## 0.0 内容分区规则：Hermes/玄成 与 OpenClaw/阿爪/taizi 严格分栏


- 2026-05-11 10:02（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：按用户点名补齐 jq 并重跑 clean replay / official overlay replay，最终验收从“带 notes”升级为全绿盖章。** 本轮先安装宿主工具 `jq`（`/usr/bin/jq`, `jq-1.6`），随后复跑 `bash scripts/doctor_edict.sh`，结果从上一轮 `PASS=31 WARN=1 FAIL=0` 变为 `PASS=32 WARN=0 FAIL=0`；唯一工具缺口已清零。随后重跑 `bash overlays/edict/scripts/replay-official-overlay.sh`。第一次 replay 在 P2 behavior pytest 阶段暴露 3 个失败：`tests/test_server.py::test_save_tasks_syncs_governance_samples_after_write` 与 `tests/test_sync_governance_samples.py` 两条样本聚合测试，根因不是生产逻辑坏，而是测试样本硬编码 `2026-04-26`，到 2026-05-11 已超过 `sync_governance_samples.py` 的 `MAX_AGE_DAYS=14`，被 `prune_samples()` 正常剪枝。已修测试口径：`tests/test_sync_governance_samples.py` 和 `tests/test_server.py` 改用当前 UTC 相对时间生成近期样本，并恢复 `test_server.py` 的 `sys.path.insert` 测试入口；focused 验证 `python3 -m pytest -q tests/test_server.py::test_save_tasks_syncs_governance_samples_after_write tests/test_sync_governance_samples.py` → `5 passed in 0.24s`。第二次 clean replay 成功：官方 baseline `14a2075`，P0/P1/P2 连续 apply，frontend baseline build 与 P0 build 通过，P0 后 `npm audit --audit-level=moderate` 为 `found 0 vulnerabilities`，最终 behavior 命令 `python3 -m pytest tests/test_server.py tests/test_autopsy_draft.py tests/test_sync_governance_samples.py -q` → `53 passed in 33.49s`；summary 为 `overlays/edict/scripts/replay-20260511-100035.summary.json`，`status=passed`，`p0_files=31 / p1_files=13 / p2_files=17`。边界：本轮没有改生产服务、没有写 DB、没有发外部消息；只安装 `jq` 与修正随时间过期的测试夹具。当前盖章口径：doctor 无 WARN，official clean replay 全绿，全面提升主线的 live runtime + replay 证据均已补齐。

- 2026-05-11 09:42（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：全面提升最终总验收已通过，可以盖章为主线收口完成。** 本轮不再扩功能，只做最终验收：dashboard `/healthz` 返回 `status=ok`，backend `/health` 返回 `ok`，deep health 返回 `postgres=true / redis=true`；`doctor_edict.sh` 返回 `PASS=31 WARN=1 FAIL=0`，唯一 WARN 仍是宿主缺 `jq`，不影响主链。三面一致性：backend / `data/tasks_source.json` / `data/live_status.json` 均 20 条，状态均为 `Done=5 / Cancelled=15`，`live_status.taskSource=backend_api_export`，`taskSourceMeta.count=20`。OpenClaw session 观测面：`python3 scripts/monitor_openclaw_sessions.py --agents taizi zhongshu menxia shangshu --json` 返回 `high=0 / medium=0 / low=0 / issues=[]`。关键回归：`python3 -m pytest -q tests/test_review_action_routes.py tests/test_backend_review_action.py tests/test_export_backend_tasks_to_legacy_json.py tests/test_refresh_live_data_layers.py tests/test_sidecar_scripts.py` → `22 passed in 3.38s`。当前 repo 在盖章前仍只剩本条主板与 ledger 回写，代码/服务主链已经干净；上一阶段已完成未跟踪项清零、远端对齐、session 噪音清零、治理字段 create 入口修复、真实字段保留样本三面验收。盖章口径：edict 全面提升主线已从大开发态收束到可运行、可观测、可回放证据充足的稳定态；后续 PWA/Notion/Linear/移动端等属于 P2 后置增强，不再阻塞“全面提升主线完成”。

- 2026-05-11 09:36（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：治理字段真实入口缺口已修复，并用第二条自然样本完成三面字段保留验收。** 上一轮真实样本 `3738719d-640f-4d4f-a754-5a280590ddd7 / JJC-20260511-NATURAL-SAMPLE` 已证明状态机可走通但 `/api/tasks` 会丢根级 `templateId/templateParams/targetDept/pending_confirm/notifications`；本轮按 TDD 先补 route 级红灯测试 `tests/test_review_action_routes.py::test_create_task_route_preserves_root_governance_fields`，首次运行失败点为 `templateId == ''`。随后修复 `edict/backend/app/api/tasks.py` 与 `edict/backend/app/services/task_service.py`：`TaskCreate` 接收根级治理字段，create route 将这些字段合并进 `meta`，`TaskService.create_task()` 同步写入模型列 `template_id/template_params/target_dept` 并保留 `pending_confirm/notifications/gate_checks`。代码层验证：目标红灯测试转绿；联跑 `tests/test_review_action_routes.py tests/test_backend_review_action.py tests/test_export_backend_tasks_to_legacy_json.py tests/test_refresh_live_data_layers.py` → `18 passed in 3.76s`；`py_compile` 通过。服务层变更前已备份到 `/root/.hermes/backups/edict-governance-field-create-fix-20260511-093545/`，随后重启 `edict-backend-api.service`，`/health` OK。真实验收样本为 `e1026bd3-5150-48aa-b418-2544ffc09984 / JJC-20260511-FIELD-PASS`，创建后立即可见 `templateId=governance-sample-pending-confirm-v1`、`templateParams.purpose=field-preservation-post-fix`、`targetDept=工部`、`pending_confirm.required=true`、`notifications.pending_confirm_sent=false`、`gate_checks[0].gate=api-create-preserve`；随后走完 `Taizi → Zhongshu → Menxia → Assigned → Doing → Review → PendingConfirm → Done`。等待 export 后三面一致：backend / `tasks_source.json` / `live_status.json` 均 20 条，状态 `Done=5 / Cancelled=15`，`taskSource=backend_api_export`；三面均保留 `templateId/templateParams/targetDept/notifications/gate_checks`。边界：`pending_confirm` 在创建后保留，最终 `Done` 后被状态机消费清空，属于预期行为，不再视为字段丢失。当前总进度口径可上调：P0 必收的仓库/提交面、三面一致、session 噪音、治理字段真实入口与字段保留样本均已收口；剩余主要是最终总验收/doctor/clean replay 或后置增强。

- 2026-05-11 09:23（北京时间）：**OpenClaw/阿爪/taizi/edict 主线：运行态验收继续推进，session 噪音已清零；真实自然样本暴露出治理字段入口缺口。** 本轮先确认主链健康：dashboard `/healthz`、backend `/health`、deep health 均 OK，`doctor_edict.sh` 为 `PASS=31 WARN=1 FAIL=0`；三面一致为 backend / `tasks_source.json` / `live_status.json` 均 19 条，状态 `Done=4 / Cancelled=15`，`live_status.taskSource=backend_api_export`。随后按“噪音也清理”的要求处理 OpenClaw session 观测面：先备份到 `/root/.hermes/backups/openclaw-session-noise-clean-20260511-092210/`，把 5 个空 transcript 历史空壳 session 从 `sessions.json` 索引移出，并给 3 个已 done 的历史 aborted session 清理 `abortedLastRun` 噪音、写入 `metadata.noiseCleared*`；为避免旧 transcript 尾部 `openclaw:prompt-error` 继续被误报成当前故障，已修改 `scripts/monitor_openclaw_sessions.py`，对已标记 `noiseClearedBy` 且非 running 的历史 session 跳过 model_error 报警，并新增 `tests/test_sidecar_scripts.py::test_monitor_ignores_model_error_for_done_session_after_noise_clear`。验证证据：`python3 -m pytest -q tests/test_sidecar_scripts.py` → `4 passed in 0.03s`；`python3 scripts/monitor_openclaw_sessions.py --agents taizi zhongshu menxia shangshu --json` → `high=0 / medium=0 / low=0 / issues=[]`。真实治理样本方面：已在备份 `/root/.hermes/backups/edict-natural-sample-20260511-092023/` 后创建低风险样本 `3738719d-640f-4d4f-a754-5a280590ddd7 / JJC-20260511-NATURAL-SAMPLE`，并走完 `Taizi → Zhongshu → Menxia → Assigned → Doing → Review → PendingConfirm → Done`；三面均能看到该样本且状态为 `Done`。但验收同时抓到真实缺口：`/api/tasks` 的 `TaskCreate` 只接收 `title/description/priority/assignee_org/creator/tags/meta`，根级 `targetDept/templateId/templateParams/pending_confirm/notifications` 会被 Pydantic 丢弃，因此三面里的样本这些字段仍为空或 null；这说明状态机和 backend export 主链可用，但“自然治理样本字段厚度”还没补齐。当前总进度口径：仓库/提交面、三面一致、session 观测噪音已基本收口；剩余 P0 必收缺口是治理字段入口/字段映射补齐 + 再跑一条字段保留真实样本，随后做最终验收记录。
